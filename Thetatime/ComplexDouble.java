...
public class ComplexDouble
{
  ...
  return new ComplexDouble(realPart, imagPart);
    ...
    public ComplexDouble prod(double... nums)
    {
  double imagPart = this.imag;
  double realPart = this.real;
  for ( double n : nums)
...
(re^itheta) ^(a+bi) = r^(a+bi) * e ^(i theta (a + bi))
  ...
  return a.prod(b);
    ...
    {
      return new ComplexDouble(Math.exp(this.real),this.imag,true);
      
    }
    public ComplexDouble sin()
    {
      return this.prod(new ComplexDouble(0,1)).exp().sum(
             this.prod(new ComplexDouble(0,-1)).exp().prod(-1)).prod(new ComplexDouble(0,.5));
    }
    public ComplexDouble cos()
    {
      return this.exp().sum(this.prod(-1).exp().prod(-1)).prod(.5);
    }
    public ComplexDouble ln()
    {
      return new ComplexDouble(Math.log(this.abs2())/2,this.imag,true);
    }
    public ComplexDouble log(ComplexDouble n)
    {
      return this.ln().prod(n.ln().mulInverse());
    }
